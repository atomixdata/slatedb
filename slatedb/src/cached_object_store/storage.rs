...
pub trait LocalCacheEntry: Send + Sync + std::fmt::Debug + 'static {
    ...
    /// startup to preload handles (and the caller's in-memory head cache) for
    /// SSTs that are already on disk. Best-effort: handles that fail to open are
    /// skipped; a missing head returns `None`.
